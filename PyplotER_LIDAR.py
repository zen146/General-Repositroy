...
def plot(frame):
    ...
    for i in a:
        ...
        mx_x = int(((i+.8)/1.6)*(mx_c_record-1))
        angle_y = ((y/1.5)*80)+3
        mx_y = int(((angle_y+((mx_r_record/2)-1))))
        a_a.append(i)
        r_r.append(r[count_t])
        b_b.append(mx_y)
    ...
    if enable_2dscan:
        ...
        Xlabel='LIDAR ANGLE, 0-90 DEG'
        Ylabel='HEAD ANGLE, 0-90 DEG'
        fw = 'bold'
        Cmap='viridis'
        axis[0].clear()
    ...
    if enable_polarscan:
        ...
        if axis_i<= 3:
            ...
            axis[1].scatter(a_a,r_r,s=20,c=c_high)
            axis[1].set_thetamin(min_angle)
            axis[1].set_thetamax(max_angle)
            axis[1].set_theta_direction(-1)
